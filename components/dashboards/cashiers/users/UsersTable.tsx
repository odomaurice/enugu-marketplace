...
const columns: ColumnDef<UserWithRelations>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Link href={`/agent-dashboard/users/${user.id}`} legacyBehavior passHref>
...
export function AgentUsersTable({ initialUsers, token }: AgentUsersTableProps) {
  const router = useRouter();

  const { data: users = initialUsers, isLoading, isError, error } = useQuery({
    queryKey: ["agent-users", token],
    queryFn: async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/agent/users`,
          {
            headers: { 
