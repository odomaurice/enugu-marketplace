...
export default async function  AgentDashboard() {
  ...
  if (!session?.user) {
    redirect(`/agent-login?callbackUrl=${encodeURIComponent('/agent-dashboard')}`);
  }
  ...
  const [usersResponse, productsResponse, ordersResponse] = await Promise.allSettled([
    axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/agent/users`, {
      headers: { Authorization: `Bearer ${session.user.token}` }
    }),
    axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/agent/products`, {
      headers: { Authorization: `Bearer ${session.user.token}` }
    }),
    axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/agent/all-order`, {
      headers: { Authorization: `Bearer ${session.user.token}` }
    })
  ]);
  ...
  return (
    <div className="p-4 mt-[60px] md:mt-[20px]">
      <h1 className="text-2xl font-bold">Welcome to your dashboard, Officer {session.user.name}.</h1>
